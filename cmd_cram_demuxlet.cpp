...
int32_t cmdCramDemuxlet(int32_t argc, char** argv) {
  ...
  */
  ...
  hprintf(wsingle, "BARCODE");
  for(int32_t j=0; j < vr.get_nsamples(); ++j){
    hprintf(wsingle, "\t%s",vr.get_sample_id_at(j));
  }
  hprintf(wsingle, "\n");
  ...
  for(std::map<std::string,int32_t>::iterator it0 = scl.bc_map.begin(); it0 != scl.bc_map.end(); ++it0, ++ncells) {
    ...
    hprintf(wsingle, "%s",it0->first.c_str());
	...
	sngNextLLK = llksAB[(j-jbeg)*nv*nAlpha];
      ...
      hprintf(wsingle, "\t%.4lf",llksAB[(j-jbeg)*nv*nAlpha]);
